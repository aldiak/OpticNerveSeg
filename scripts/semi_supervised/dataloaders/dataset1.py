...
class MDM(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image4 = h5f['image_md'][:]
        label = h5f['label'][:]
        print(f"Index {idx} → image shape: {image.shape}, image 1 shape: {image1.shape}.  mask shape: {label.shape if label is not None else 'None'}")
        sample = {'image': image, 'image1': image1, 'image2': image2, 'image3': image3, 'image4': image4, 'label': label}
        if self.transform:
            sample = self.transform(sample)
        
        # ── ADD THIS ───────────────────────────────────────
        print(f"Idx {idx:3d} | ", end="")
        for k, v in sample.items():
            shape_str = str(v.shape) if hasattr(v, 'shape') else str(type(v))
            print(f"{k}: {shape_str}  ", end="")
        print()  # new line
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        image, image1, image2, image3, image4, label = sample['image'], sample['image1'], sample['image2'], sample['image3'], sample['image4'], sample['label']
        if self.with_sdf:
            sdf = sample['sdf']
        ...
        if self.with_sdf:
            ...
            return {'image': image, 'image1': image1, 'image2': image2, 'image3': image3, 'image4': image4, 'label': label, 'sdf': sdf}
        else:
            return {'image': image, 'image1': image1, 'image2': image2, 'image3': image3, 'image4': image4, 'label': label}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        image, image1, image2, image3, image4 = sample['image'], sample['image1'], sample['image2'], sample['image3'], sample['image4']
        ...
        if 'onehot_label' in sample:
            return {'image': torch.from_numpy(image), 'image1': torch.from_numpy(image1), 'image2': torch.from_numpy(image2), 'image3': torch.from_numpy(image3), 'image4': torch.from_numpy(image4), 'label': torch.from_numpy(sample['label']).long(),
