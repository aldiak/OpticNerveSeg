...
def var_all_case_HCP(
    ...
    for i_batch, sampled_batch in enumerate(valloader):
        ...
        prediction, score_map = test_single_case1(
            model, image, image1, stride_xy, stride_z, patch_size, num_classes=num_classes
        )
        if np.sum(prediction) == 0:
            dice = 0
        else:
            dice = metric.binary.dc(prediction, label.cpu().detach().numpy())
        total_dice += dice
    avg_dice = total_dice / len(valloader)
    print("average metric is {}".format(avg_dice))
    return avg_dice

def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    with open('/mnt/disk1/UnCo/code_upload/data_split/MDM/val.list', 'r') as f:
        image_list = f.readlines()
    image_list = ['/mnt/disk1/UnCo/code_upload/data_split/MDM' + '/data/' + item.replace('\n', '').split(".")[0] + '.h5' for item in image_list]
...
def var_all_case_BTCV(model, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=16):
    ...
    for image_path in loader:
        ...
        prediction, score_map = test_single_case_btcv(model, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
        if np.sum(prediction) == 0:
...
def test_single_case1(net, image, image1, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                test_patch1 = np.expand_dims(np.expand_dims(test_patch1, axis=0), axis=0).astype(np.float32)
                test_patch1 = torch.from_numpy(test_patch1).cuda()

                with torch.no_grad():
                    y, y1, _ , _, _ = net(torch.cat((test_patch, test_patch1), dim=1), [])
                    # ensemble
                    #y1 = (y + y1)/2
...
def test_single_case_btcv(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
