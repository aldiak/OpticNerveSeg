...
class HCP(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image1 = h5f['image_t2'][:]
        label = h5f['label'][:]
        sample = {'image': image, 'image1': image1, 'label': label}
        if self.transform:
            sample = self.transform(sample)
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        image, image1, label = sample['image'], sample['image1'], sample['label']
        if self.with_sdf:
            sdf = sample['sdf']
        ...
        if self.with_sdf:
            ...
            return {'image': image, 'image1': image1, 'label': label, 'sdf': sdf}
        else:
            return {'image': image, 'image1': image1, 'label': label}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        image, image1 = sample['image'], sample['image1']
        image = image.reshape(1, image.shape[0], image.shape[1], image.shape[2]).astype(np.float32)
        image1 = image1.reshape(1, image1.shape[0], image1.shape[1], image1.shape[2]).astype(np.float32)
        if 'onehot_label' in sample:
            return {'image': torch.from_numpy(image), 'image': torch.from_numpy(image1), 'label': torch.from_numpy(sample['label']).long(),
                    'onehot_label': torch.from_numpy(sample['onehot_label']).long()}
        else:
            return {'image': torch.from_numpy(image), 'image1': torch.from_numpy(image1),'label': torch.from_numpy(sample['label']).long()}
